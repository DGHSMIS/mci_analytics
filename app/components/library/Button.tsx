...
interface ButtonParams {
  ...
  clicked?: MouseEventHandler<MouseEvent>;
  renderAs?: "a" | "Link";
  anchor?: {
    key?: Key | null;
    href: string;
    ...
    return (
      <>
        ...
        {!isDisabled &&
          hasSpinner &&
          (size == "sm" ? (
            <Spinner size={"sm"} variant="primary" trackColor="white" />
          ) : size == "md" ? (
            <Spinner size={"md"} variant="primary" trackColor="white" />
          ) : size == "lg" ? (
            <Spinner size={"lg"} variant="primary" trackColor="white" />
          ) : (
            ""
          ))}

        {/* //+ Button Text */}
        <span className={cn("whitespace-nowrap leading-0", textClassName)}>
          {btnText}
        </span>
